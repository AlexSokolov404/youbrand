document.addEventListener('DOMContentLoaded', () => {
  ...
  const getProductsByCategory = async (key) => {
    ...
    return fetch(`component/service-ready-decisions/${key}.json`)
      .then(res => res.json()
        .then(data => {
          catalogWrapper.innerHTML = ''
          data.map(item => {
            catalogWrapper.insertAdjacentHTML('beforeend', `
            <div class="decisionsGrid__wrapper-item" data-id=${item.id}>
                <div class="decisionsGrid__wrapper-item-imgWrapper">
                
                    <a href=${item.url}>
                        <!-- Default image -->
                        <img loading="lazy" src=${item.defaultImg} alt="catalog-item" class="decisionsGrid__wrapper-item-imgWrapper-defaultImg">
                        <!-- Image on hover -->
                        <img loading="lazy" src=${item.hoverImg} alt="catalog-item" class="decisionsGrid__wrapper-item-imgWrapper-hoverImg">
                    </a>
                    
                    
                    <!-- Colored boxes -->
                    <div class="decisionsGrid__wrapper-item-offerBox">
                        ${item.isNew ? `<div class="decisionsGrid__wrapper-item-offerBox-green">Новинка</div>` : ''}
                        ${item.isBestseller ? `<div class="decisionsGrid__wrapper-item-offerBox-yellow">Бестселлер</div>` : ''}
                        ${item.isDiscounted ? `<div class="decisionsGrid__wrapper-item-offerBox-red">Скидка ${item.discountSize}%</div>` : ''}
                    </div>

                    <!-- Elements on hover -->
                    <button class="decisionsGrid__wrapper-item-button">В корзину</button>
                    
                    <!-- 'Add to favorites' icon -->
                    <div class="decisionsGrid__wrapper-item-favoriteIconWrapper 
                         ${key === 'favoritesPageData'
              ? 'decisionsGrid__wrapper-item-favoriteIconWrapper--favoritesPage'
              : null}">
                        <img src="component/catalogGrid/img/favorite-icon.svg" alt="favorite" class="decisionsGrid__wrapper-item-favoriteIconWrapper-icon">

                        <!-- Tooltip -->
                        <div class="decisionsGrid__wrapper-item-favoriteIconWrapper-tooltip">
                            ${key === 'favoritesPageData'
              ...
              : 'В избранное'}
                ...
                <div class="decisionsGrid__wrapper-item-info">
                    <a href=${item.url} class="decisionsGrid__wrapper-item-info-title">${item.title}</a>

                    <div class="decisionsGrid__wrapper-item-info-prices">
                        <div class="decisionsGrid__wrapper-item-info-prices-main">${item.newPrice} ₽</div>
                        ${item.oldPrice ? `<div class="decisionsGrid__wrapper-item-info-prices-old">${item.oldPrice} ₽</div>` : ''}
                    </div>

                <!-- Colors -->
                <div class="decisionsGrid__wrapper-item-info-colorIcons">
                    
                    ${item.colors?.map(color => {
                        return(`
                        <div class="decisionsGrid__wrapper-item-info-colorIcons-item" data-color=${color.id}>
                            <div class="decisionsGrid__wrapper-item-info-colorIcons-item-default decisionsGrid__wrapper-item-info-colorIcons-item-defaultActive">
                               <div class="decisionsGrid__wrapper-item-info-colorIcons-item-default-inner" style='background-color: ${color.code}'></div>
                        </div>
                        <!-- Tooltip -->
                        <div class="decisionsGrid__wrapper-item-info-colorIcons-item-tooltip">
                            ${color.name}
                        </div>
                    </div>
                        `)
      ...
      .finally(() => {
        ...
        const cards = document.querySelectorAll('.decisionsGrid__wrapper-item')
        cards.forEach(card => {
          const colorBoxes = card.querySelectorAll('.decisionsGrid__wrapper-item-info-colorIcons-item-default')

          const removeActiveColor = () => {
            colorBoxes.forEach(item => {
              item.classList.remove('decisionsGrid__wrapper-item-info-colorIcons-item-defaultActive')
            })
          }
          ...
          colorBoxes.forEach(item => {
            item.addEventListener('click', () => {
              removeActiveColor()
              item.classList.add('decisionsGrid__wrapper-item-info-colorIcons-item-defaultActive')
            })
          })
        })
  ...
  const catalogWrapper = document.querySelector('.decisionsGrid__wrapper')
  ...
  let colorBoxes
  const getColorBoxes = () => {
    colorBoxes = document.getElementsByClassName('decisionsGrid__wrapper-item-info-colorIcons-item')
    ...
    fetch(`component/catalogGrid/colorImages.json?` + new URLSearchParams({
      ...
      .then(res => res.json()
        .then(data => {
          ...
          const defaultImg = thisProduct.querySelector('.decisionsGrid__wrapper-item-imgWrapper-defaultImg')
          const hoverImg = thisProduct.querySelector('.decisionsGrid__wrapper-item-imgWrapper-hoverImg')
  ...
  const addToCartLogic = () => {
    ...
    // open logic
    const addToCartButtons = document.querySelectorAll('.decisionsGrid__wrapper-item-button')
