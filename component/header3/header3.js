...
document.addEventListener('mousemove', e => {
  const currentNavContainer = e.target.closest('.header3__nav')
  const navItem = e.target.closest('.header3__nav-menuItem')
  const currentDropDown = e.target.closest('[data-dropdown-category]')
  let mouseOnNavigation = false

  if (currentNavContainer) {
    mouseOnNavigation = true
    if (navItem && navItem.hasAttribute('data-nav-category')) {
      ...
      /* Close if navItem has no dropdown (has own data-nav-category value) */
      if (navItem?.getAttribute('data-nav-category') === 'none') {
    ...
    } else if ((!navItem?.hasAttribute('data-nav-category') && !mouseOnNavigation)) {
      dropdownMenu.forEach(item => {
        item?.classList.add('hidden')
      })
      dropdownParent.classList.add('hidden')
    }
  } else if (!mouseOnNavigation && !currentDropDown) {
    dropdownMenu.forEach(item => {
      item?.classList.add('hidden')
    })
    dropdownParent.classList.add('hidden')
      ...
      resolve([
        {
          ...
          imageSrc: 'component/header3/img/found-products/blue.jpg',
          newPrice: 3000,
          oldPrice: 5000,
          productPageUrl: '#',
        },
